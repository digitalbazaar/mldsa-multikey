/*!
 * Copyright (c) 2023-2026 Digital Bazaar, Inc.
 */
import {sha256} from '@noble/hashes/sha2.js';
import * as base58 from 'base58-universal';
import * as base64url from 'base64url-universal';
import {
  ALGORITHM,
  MULTIBASE_BASE64URL_HEADER,
  MULTICODEC_MLDSA44_PUBLIC_KEY_HEADER,
  MULTICODEC_MLDSA65_PUBLIC_KEY_HEADER,
  MULTICODEC_MLDSA87_PUBLIC_KEY_HEADER,
  MULTICODEC_MLDSA44_SECRET_KEY_SEED_HEADER,
  MULTICODEC_MLDSA65_SECRET_KEY_SEED_HEADER,
  MULTICODEC_MLDSA87_SECRET_KEY_SEED_HEADER,
} from './constants.js';

// retrieves algorithm name from public Multikey header bytes
export function getAlgorithmFromPublicMultikey({publicMultikey}) {
  if(publicMultikey[0] === MULTICODEC_MLDSA44_PUBLIC_KEY_HEADER[0] &&
    publicMultikey[1] === MULTICODEC_MLDSA44_PUBLIC_KEY_HEADER[1]) {
    return ALGORITHM.MLDSA44;
  }
  if(publicMultikey[0] === MULTICODEC_MLDSA65_PUBLIC_KEY_HEADER[0] &&
    publicMultikey[1] === MULTICODEC_MLDSA65_PUBLIC_KEY_HEADER[1]) {
    return ALGORITHM.MLDSA65;
  }
  if(publicMultikey[0] === MULTICODEC_MLDSA87_PUBLIC_KEY_HEADER[0] &&
    publicMultikey[1] === MULTICODEC_MLDSA87_PUBLIC_KEY_HEADER[1]) {
    return ALGORITHM.MLDSA87;
  }

  throw new TypeError('Unsupported public multikey header.');
}

// retrieves algorithm name from secret Multikey header bytes
export function getAlgorithmFromSecretMultikey({secretMultikey}) {
  if(secretMultikey[0] === MULTICODEC_MLDSA44_SECRET_KEY_SEED_HEADER[0] &&
    secretMultikey[1] === MULTICODEC_MLDSA44_SECRET_KEY_SEED_HEADER[1]) {
    return ALGORITHM.MLDSA44;
  }
  if(secretMultikey[0] === MULTICODEC_MLDSA65_SECRET_KEY_SEED_HEADER[0] &&
    secretMultikey[1] === MULTICODEC_MLDSA65_SECRET_KEY_SEED_HEADER[1]) {
    return ALGORITHM.MLDSA65;
  }
  if(secretMultikey[0] === MULTICODEC_MLDSA87_SECRET_KEY_SEED_HEADER[0] &&
    secretMultikey[1] === MULTICODEC_MLDSA87_SECRET_KEY_SEED_HEADER[1]) {
    return ALGORITHM.MLDSA87;
  }

  throw new TypeError('Unsupported secret multikey header.');
}

// sets secret key seed header bytes on key pair
export function setSecretKeySeedHeader({algorithm, buffer}) {
  if(algorithm === ALGORITHM.MLDSA44) {
    buffer[0] = MULTICODEC_MLDSA44_SECRET_KEY_SEED_HEADER[0];
    buffer[1] = MULTICODEC_MLDSA44_SECRET_KEY_SEED_HEADER[1];
  } else if(algorithm === ALGORITHM.MLDSA65) {
    buffer[0] = MULTICODEC_MLDSA65_SECRET_KEY_SEED_HEADER[0];
    buffer[1] = MULTICODEC_MLDSA65_SECRET_KEY_SEED_HEADER[1];
  } else if(algorithm === ALGORITHM.MLDSA87) {
    buffer[0] = MULTICODEC_MLDSA87_SECRET_KEY_SEED_HEADER[0];
    buffer[1] = MULTICODEC_MLDSA87_SECRET_KEY_SEED_HEADER[1];
  } else {
    throw new TypeError(`Unsupported algorithm "${algorithm}".`);
  }
}

// creates a base58btc-encoded multihash key id fragment from a base64url
// multibase-encoded public key
export function publicKeyMultibaseToKeyId({publicKeyMultibase}) {
  // strip multibase header and decode to get multicodec-prefixed key bytes
  if(publicKeyMultibase[0] !== MULTIBASE_BASE64URL_HEADER) {
    throw new TypeError(
      '"publicKeyMultibase" must use the base64url multibase encoding.');
  }
  const multikey = base64url.decode(publicKeyMultibase.slice(1));
  // strip 2-byte multicodec header to get raw public key bytes
  const publicKeyBytes = multikey.subarray(2);
  return publicKeyBytesToKeyId({publicKeyBytes});
}

// creates a base58btc-encoded multihash key id fragment from raw public key
// bytes; uses SHA-256 (0x12) multihash header
export function publicKeyBytesToKeyId({publicKeyBytes}) {
  const digest = sha256(publicKeyBytes);
  const multihash = new Uint8Array(2 + digest.length);
  multihash[0] = 0x12;
  multihash[1] = 0x20;
  multihash.set(digest, 2);
  return 'z' + base58.encode(multihash);
}

// sets public key header bytes on key pair
export function setPublicKeyHeader({algorithm, buffer}) {
  if(algorithm === ALGORITHM.MLDSA44) {
    buffer[0] = MULTICODEC_MLDSA44_PUBLIC_KEY_HEADER[0];
    buffer[1] = MULTICODEC_MLDSA44_PUBLIC_KEY_HEADER[1];
  } else if(algorithm === ALGORITHM.MLDSA65) {
    buffer[0] = MULTICODEC_MLDSA65_PUBLIC_KEY_HEADER[0];
    buffer[1] = MULTICODEC_MLDSA65_PUBLIC_KEY_HEADER[1];
  } else if(algorithm === ALGORITHM.MLDSA87) {
    buffer[0] = MULTICODEC_MLDSA87_PUBLIC_KEY_HEADER[0];
    buffer[1] = MULTICODEC_MLDSA87_PUBLIC_KEY_HEADER[1];
  } else {
    throw new TypeError(`Unsupported algorithm "${algorithm}".`);
  }
}
