...
import {exportKeyAsJwk, generateKey} from './crypto.js';
import {
  cryptoKeyfromRaw,
  exportKeyPair,
  importKeyPair,
...
export async function toJwk({keyPair, secretKey = false} = {}) {
  ...
  const key = useSecretKey ? keyPair.secretKey : keyPair.publicKey;
  return exportKeyAsJwk(key);
}
...
export async function fromRaw({
  ...
  const cryptoKey = cryptoKeyfromRaw({algorithm: algorithmName,
    secretKey, publicKey});
  const jwk = exportKeyAsJwk(cryptoKey);
