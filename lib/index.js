...
import {exportKeyAsJwk, generateKey, importKey} from './mldsa.js';
import {
  exportKeyPair,
  importKeyPair,
...
export async function generate({
  id, controller, algorithm = ALGORITHM.MLDSA44
} = {}) {
  const keyPair = generateKey({algorithm});
...
export async function from({key} = {}) {
  ...
  if(multikey.controller && !multikey.id) {
    multikey.id = `${multikey.controller}#${
      publicKeyMultibaseToKeyId(
...
export async function fromJwk({jwk, secretKey = false, id, controller} = {}) {
  ...
  if(secretKey && jwk.priv) {
    ...
    const key = importKey({format: 'jwk', keyData: jwk, algorithm});
    multikey.secretKeyMultibase = secretKeyToMultibase({key});
  }
  return from({key: multikey});
}
...
export async function toJwk({keyPair, secretKey = false} = {}) {
  ...
  if(useSecretKey) {
    ...
    const jwk = exportKeyAsJwk({key: keyPair.secretKey});
    jwk.pub = exportKeyAsJwk({key: keyPair.publicKey}).pub;
    return jwk;
  }
  return exportKeyAsJwk({key: keyPair.publicKey});
}

// raw import from publicKey bytes and optional secretKey seed bytes
export async function fromRaw({
  algorithm = ALGORITHM.MLDSA44, secretKey, publicKey
} = {}) {
  ...
  const pubKey =
    importKey({format: 'raw-public', keyData: publicKey, algorithm});
  const multikey = {
    '@context': MULTIKEY_CONTEXT_V1_URL,
    type: 'Multikey',
    publicKeyMultibase:
      toPublicKeyMultibase({jwk: exportKeyAsJwk({key: pubKey})})
  };
  if(secretKey) {
    // secretKey must be the 32-byte seed
    const secKey =
      importKey({format: 'raw-seed', keyData: secretKey, algorithm});
