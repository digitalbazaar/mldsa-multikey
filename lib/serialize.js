...
// imports raw key bytes as key objects
export function cryptoKeyfromRaw(
  {algorithm: algorithmName, secretKey, publicKey} = {}) {
  if(secretKey) {
    return importKey('raw-secret', secretKey, algorithmName);
  }
  return importKey('raw-public', publicKey, algorithmName);
}
...
export async function exportKeyPair({
  ...
  if(secretKey && keyPair.secretKey) {
    const {name: algorithm} = keyPair.secretKey.algorithm;
    const secretKeyBytes = new Uint8Array(exportKeyBytes(keyPair.secretKey));
    // leave room for multicodec header (2 bytes)
    const multikey = new Uint8Array(2 + secretKeyBytes.length);
    setSecretKeyHeader({algorithm, buffer: multikey});
...
export function importKeyPair({
  ...
  const algorithmName = getAlgorithmFromPublicMultikey({publicMultikey});
  keyPair.publicKey = importKey('raw-public', rawPublicKey, algorithmName);
  ...
  if(secretKeyMultibase) {
    ...
    // ensure secret key multikey header matches the public key multikey header
    ...
    keyPair.secretKey = importKey('raw-secret', secretKeyBytes, algorithmName);
  }
