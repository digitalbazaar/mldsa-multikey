...
import {exportKeyAsJwk, importKey} from './crypto.js';
import {
...
export async function exportKeyPair({
  ...
  if(secretKey && keyPair.secretKey) {
    exported.secretKeyMultibase = secretKeyToMultibase({key: keyPair.secretKey});
  }
...
export function importKeyPair({
  ...
  if(secretKeyMultibase) {
    ...
    keyPair.secretKey = importKey(
      'raw-seed', secretMultikey.subarray(2), algorithmName);
  }
