...
// Algorithm name constant used throughout the codebase
const MLDSA_ALGORITHM_NAME = 'MLDSA';
...
const NIST_LEVEL_MAP = new Map([
  [2, {impl: ml_dsa44, algName: 'ML-DSA-44'}],
]);
...
class SubtleCrypto {
  ...
  async generateKey(algorithm, extractable, keyUsages) {
    ...
    const keyAlgorithm = {name: MLDSA_ALGORITHM_NAME, nistSecurityLevel};
  ...
  async exportKey(format, key) {
    ...
    const {algName} = _getImpl(nistSecurityLevel);
...
// Returns the noble implementation and algName for a given NIST security level
...
function _importJwk({
  ...
  if(jwk.alg && jwk.alg !== algName) {
    throw new DOMException(
      `JWK "alg" must be "${algName}", got "${jwk.alg}".`, 'DataError');
  }
