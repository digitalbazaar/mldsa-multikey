...
export function sign({secretKey, data} = {}) {
  ...
  _assertKey(secretKey, 'private');
  const {impl} = _getImpl(secretKey.algorithm);
  const sig = impl.sign(data, secretKey._keyBytes);
...
export function verify({publicKey, signature, data} = {}) {
  ...
  _assertKey(publicKey, 'public');
  const {impl} = _getImpl(publicKey.algorithm);
  try {
    return impl.verify(signature, data, publicKey._keyBytes);
  } catch(e) {
    return false;
  }
}
...
export function importKey({format, keyData, algorithm} = {}) {
  ...
  if(format === 'raw-seed') {
    ...
    const {secretKey: secBytes} = impl.keygen(keyData);
    return {
      type: 'private',
      algorithm,
      _keyBytes: secBytes,
...
function _assertKey(key, expectedType) {
